<?php
	...
	class CamControl extends Webcam {
		...
		public function CamControl () {
			...
			if (isset($this->_formData['seconds_active'])) {
				$this->secondsActive = intval($this->_formData['seconds_active']);
			} else {
		...
		public function pulse () {
			...
			/********************************
			MAKE-SHIFT FIX FOR DB PROBLEM TO MEET CLIENT DEMAND FOR CAM CONTROL RE-IMPLEMENTATION
			Even though the 'control_queue' table is only indexed with a single primary key, UPDATE queries on the table take longer and longer
			to run as the table size grows. The deletion of inactive, unqueued control_queue records must also be supplemented by an OPTIMIZE TABLE
			call to "defrag" MySQL memory for the table as records are created and deleted over time.
			********************************/
			// "hard prune" the queue
			$query = "DELETE FROM control_queue WHERE in_queue = 0 AND uid_cam = {$this->_uid_cam}";
			$resource = $this->getDBResource($query);
			// now let's optimize any table in current 'webcam' DB that needs it (effectively will always be 'control_queue')
			// this is an expensive query that might best be run via CRON, but this way we aim for real-time prevention of attack (intetnional or unintentional) of DB
			$query = "SHOW TABLE STATUS WHERE Data_free / Data_length > 0.1 AND Data_free > 102400";
			$resource = $this->getDBResource($query);
			while($row = mysql_fetch_assoc($resource)) {
				mysql_query('OPTIMIZE TABLE ' . $row['Name']);
			}
			/********************************
			END OF MAKE-SHIFT FIX FOR DB PROBLEM TO MEET CLIENT DEMAND FOR CAM CONTROL RE-IMPLEMENTATION
			********************************/
